...
findLangBin lang =
  liftIO (findExecutable (Lang.defaultBinary lang)) >>= \case
    Just bin -> pure bin
    Nothing ->
      fail $ unlines $
...
parseModulePathLang path =
  case Map.lookup (takeFileName $ takeDirectory $ takeDirectory path) Lang.allLangs of
    Just lang -> pure lang
    Nothing -> fail $ unlines $
      ...
      , "Currently supported languages are " <> intercalate ", " (Map.keys Lang.allLangs) <> "."
      ]
...
parseModulePath path = do
  ...
  base <- parseModuleBaseName (Lang.fileExt lang) path
  pure (lang, n, base)
...
moduleRules =
  "_build/*/*/*" %> \out -> do
    ...
    case Map.lookup base modules of
      Just module_ ->
        writeTextFileChanged out (Lang.render lang $ module_ n)
      Nothing ->
        putError $ unlines $
...
benchmarkLangOracle benchmark BenchmarkLang{..} = do
  let dir = "_build" </> Lang.name benchLang </> show benchSize
  let file = benchModuleName -<.> Lang.fileExt benchLang
  removeFilesAfter dir (Lang.buildArtifacts benchLang)
  need [dir </> file]
  bin <- findLangBin benchLang
  benchmark (BenchmarkExec bin (Lang.defaultFlags benchLang file) [] dir)
