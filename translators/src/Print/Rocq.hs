...
printImport (ImportLib VecMod) = G.line (import_ ++ "Coq.Vectors.Vector. ") ++ "Import VectorNotations."
...
printDef (DefTVar var (Just t) expr) = 
  G.line $ "Definition " ++ var ++ typedel ++ printType t ++ assign ++ printExpr expr ++ ". "
...
printDef (DefPDataType name params args ty) = let
    ...
    in
        "Inductive " ++ map toLower name ++
         unwords (map (\(x, y) -> " " ++ G.parens (map toLower x ++ typedel ++ printType y)) params) ++
         typedel ++ printType ty ++ assign ++
...
printDef (DefRecType name params consName fields _) =
    ...
    G.line (G.brackets ("\n" ++ concatMap (\(fname, ftype) -> G.line $ "  " ++ fname ++ typedel ++ printType ftype ++ ";") fields) ++ ".")
    where
        paramsStr = case params of
            [] -> ""
            _ -> " " ++ unwords (map (\(Arg n t) -> G.parens $ n ++ " : " ++ printType t) params)

printDef (DefRec name recType consName fields) =
  G.line ("Definition " ++ name ++ typedel ++ printType recType ++ assign) ++ "  " ++ G.line (constructorCall ++ ".")
  where
...
printModule (Module name imports defs) =
    let headers =
          (if null imports then emptyDoc
                      else vsep (map (pretty . printImport) imports) <> hardline <> hardline) <>
