...
**1. Processing the JSON file from GitHub Actions**
The app loads benchmark data from `data.json` (generated by GitHub Actions), which contains timing/memory metrics for different input sizes across languages.

**2. Plotting the Data**  
The app uses Python’s `matplotlib` to create four types of performance graphs for each test case:  
- **Real Time**: CPU time or Clock time in seconds.
- **User Time**: Amount of time in seconds spent outside of the kernel.
- **System Time**: Amount of time in seconds spent inside the kernel. 
- **Memory**: Maximum resident set size of the type checking command, in Megabytes. In other words, it measures how much memory (RAM) the program used at its peak while it was running a type-checking command. This number is shown in Megabytes (MB).

**3. Error Handling**  

- Memory limits exceeded - (red X marker).  
- Time limits exceeded - (blue X marker).
- A bright blue square marker indicates that there is an artificial zero (startup time is larger than type check time) at that point. 

**4. Base64 Magic**  
Since Vercel (our hosting platform) doesn’t support saving image files, the app:  
1. Generates graphs in memory (no files saved).  
2. Converts them to **base64 text strings**.  
3. Embeds them directly into the webpage.

**5. Log plots:** If the selected interval is 'log,' take the log of each time and memory value. Hence, the y axis is the log value of the time in base 2, but if any time is equal to or less than 0, set the time to the log base 2 of 0.01. 

**6. Startup time normalization:** Subtract startup times from the recorded times to normalize the data. If the startup time is larger than the recorded time, record the time as an artificial zero. 
